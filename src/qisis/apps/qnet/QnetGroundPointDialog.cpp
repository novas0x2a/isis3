...
namespace Qisis {
  ...
  QnetGroundPointDialog::QnetGroundPointDialog (QWidget *parent) : QDialog (parent) {
    ...
    setLayout(vLayout);
    setWindowTitle("Create Ground ControlPoint");

  }

  void QnetGroundPointDialog::setPoint (Isis::ControlPoint &point) {
    p_point = &point;
  ...
  void QnetGroundPointDialog::selectMeasures () {
  ...
  void QnetGroundPointDialog::accept () {

    //  If setting as ground point ,make sure point isn't ignored, held
    p_point->SetHeld(false);
    p_point->SetIgnore(false);
    ...
    p_point->SetType(Isis::ControlPoint::Ground);
    emit groundPoint (*p_point);
