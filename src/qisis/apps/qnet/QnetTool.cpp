...
//    connect(p_modifyPoint,SIGNAL(triggered()),this,SLOT(modifyPoint()));
  ...
  /**
   * @internal
   *   @history 2008-11-24  Jeannie Walldren - Added "Goodness of
   *            Fit" to right and left measure info.
   *   @history 2008-11-26  Jeannie Walldren - Added "Number of
   *            Measures" to QnetTool point information.  Moved
   *            setWindowTitle() command to updateNet() method.
   *            Added connection between Ignore checkbox toggle()
   *            slot and ignoreChanged() signal
   *   @history 2008-12-29 Jeannie Walldren - Disabled ground
   *            point check box and commented out connection
   *            between check box and setGroundPoint() method.
   *   @history 2008-12-30 Jeannie Walldren - Added connections to
   *            toggle measures' Ignore check boxes if
   *            ignoreLeftChanged() and ignoreRightChanged() are
   *            emitted.  Replaced reference to ignoreChanged()
   *            with ignorePointChanged().
   */
...
//            p_pointEditor,SLOT(modifyPoint(Isis::ControlPoint *)));
    ...
    connect(this,SIGNAL(editPointChanged(std::string)),
            this,SLOT(paintAllViewports(std::string)));
   ...
   *  
   * @internal 
   *   @history 2008-11-26 Jeannie Walldren - Added message box to
   *            warn the user that they are saving an "Ignore"
   *            point and ask whether they would like to set
   *            Ignore=false.  This emits an ignoreChanged()
   *            signal so the "Ignore" box in the window is
   *            unchecked.
   *   @history 2008-12-30 Jeannie Walldren - Modified to set
   *            measures in viewports to Ignore=False if when
   *            saving, the user chooses to set a point's
   *            Ignore=False. Replaced reference to ignoreChanged()
   *            with ignorePointChanged().
   *   @history 2008-12-31 Jeannie Walldren - Added question box
   ...
   *  
   * @internal
   *   @history 2008-11-18 Jeannie Walldren - Added "Close" action
   *            to the file menu on the qnet tool window.
   *   @history 2008-12-10 Jeannie Walldren - Added "What's this?"
   *            function to qnet tool actions.
   */
...
//     connect (interestOp,SIGNAL(activated()),this,SLOT(setInterestOp()));
   ...
   *             set.
   * @internal 
   *   @history 2008-12-29 Jeannie Walldren - Added dialog box to
   *            allow user to determine the lat/lon/radius for
   *            this point if set to Held=True.
   */
   ...
   *   @history 2008-12-30 Jeannie Walldren - Replaced reference
   *            to ignoreChanged() with ignorePointChanged().
   */
   ...
   *   @history 2008-11-26 Jeannie Walldren - Added cNetFilename input
   *            parameter in order to show the file path in the
   *            window's title bar.
   */
  ...
  void QnetTool::mouseButtonRelease(QPoint p, Qt::MouseButton s) {
    ...
    std::string sn = g_serialNumberList->SerialNumber(file);
   ...
   * 
   */
  std::vector<std::string> QnetTool::findPointFiles(double lat,double lon) {
   ...
   *   @internal
   *   @history 2008-11-20 Jeannie Walldren - Added message box if
   *            pointID value entered already exists for another
   *            ControlPoint. Previously this resulted in a
   *            PROGRAMMER ERROR from ControlPoint. Now, the user
   *            will be alerted in a message box and prompted to
   *            enter a new value for pointID.
  ...
  void QnetTool::createPoint(double lat,double lon) {
    ...
    if (newPointDialog->exec()) {
      ...
      if (g_controlNetwork->Exists(*newPoint)) {
        std::string message = "A ControlPoint with Point Id = [" + newPoint->Id();
   ...
   * @internal 
   *   @history 2008-11-26  Jeannie Walldren - Added "Number of 
   *            Measures" to QnetTool point information.
   */
...
//    }
  ...
  void QnetTool::paintViewport (CubeViewport *vp,QPainter *painter) {
  ...
  void QnetTool::paintAllViewports (std::string pointId) {
...
//       cout<<"after paintViewport vp= "<<i<<endl;
  ...
  void QnetTool::drawAllMeasurments (CubeViewport *vp,QPainter *painter) {
    ...
    std::string serialNumber = Isis::SerialNumber::Compose(*vp->cube());
    ...
    for (int i=0; i<g_controlNetwork->Size(); i++) {
      ...
      for (int j=0; j<p.Size(); j++) {
        if (p[j].CubeSerialNumber() == serialNumber) {
          if (p[j].Ignore() && p_controlPoint != NULL &&
              p.Id() != p_controlPoint->Id()) painter->setPen(QColor(255,255,0));
