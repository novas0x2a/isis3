...
namespace Qisis {
  ...
  QWidget *EditTool::createToolBarWidget (QStackedWidget *active) {
    ...
    p_valTypeComboBox->addItem("Dn");
    p_valTypeComboBox->addItem("Null");
    p_valTypeComboBox->addItem("Hrs");
    p_valTypeComboBox->addItem("Lrs");
    p_valTypeComboBox->addItem("His");
    ...
    //  TODO::  Add validator???
    //QValidator *valid = new QValidator (hbox);
  ...
  void EditTool::removeViewport (QObject *vp) {
    ...
    if (p_undoEdit.contains((CubeViewport *) vp)) {
      QStack<Isis::Brick *> *temp = p_undoEdit.value((CubeViewport *) vp);
      while(!temp->isEmpty()) {
        delete temp->pop();
      }
      p_undoEdit.remove((CubeViewport *) vp);
    } 
    if (p_redoEdit.contains((CubeViewport *) vp)) {
      QStack<Isis::Brick *> *temp = p_redoEdit.value((CubeViewport *) vp);
      while(!temp->isEmpty()) {
        delete temp->pop();
      }
      p_redoEdit.remove((CubeViewport *) vp);
    } 
  }
