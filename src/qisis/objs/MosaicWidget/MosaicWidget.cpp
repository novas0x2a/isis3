...
namespace Qisis {
  ...
  void MosaicWidget::initWidget(){
    ...
    connect(p_controlPointButton, SIGNAL(pressed()), this, SLOT(displayControlPoints()));
     ...
     header << "Name" << "Item" << "Footprint" << "Outline" << "Image"
            ...
            << "Incidence Angle" << "Notes";
     p_treeWidget->setHeaderLabels(header);
     p_treeWidget->setColumnWidth(0,160); //Name
     p_treeWidget->setColumnWidth(1,35); //Item
     p_treeWidget->setColumnWidth(2,60); //Footprint
     p_treeWidget->setColumnWidth(3,50); //Outline
     p_treeWidget->setColumnWidth(4,45); //Image
     p_treeWidget->setColumnWidth(5,40); //Label
     p_treeWidget->setColumnWidth(6,70); //Resolution
     p_treeWidget->setColumnWidth(7,100); //Emission Angle
     p_treeWidget->setColumnWidth(8,110); //Incidence Angle
  ...
  void MosaicWidget::addItem(MosaicItem *mosaicItem, QString groupName){
   ...
   for(int i = 0; i < p_mosaicItems.size(); i++) {
    ...
    if(p_insertItemAt == -1) {
      ...
      mosaicItem->treeItem()->setText(6, QString::number(mosaicItem->pixelResolution()));
      mosaicItem->treeItem()->setText(7, QString::number(mosaicItem->emissionAngle()));
      mosaicItem->treeItem()->setText(8, QString::number(mosaicItem->incidenceAngle()));
    } else{
  ...
  void MosaicWidget::viewMenuAction(QAction *action){
    ...
    if(action->text() == "Item Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(1);
      } else {      
        p_treeWidget->hideColumn(1);
      }
    }
    if(action->text() == "Footprint Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(2);
      } else {      
        p_treeWidget->hideColumn(2);
      }
    }

    if(action->text() == "Outline Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(3);
      } else {      
        p_treeWidget->hideColumn(3);
      }
    }

    if(action->text() == "Image Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(4);
      } else {      
        p_treeWidget->hideColumn(4);
      }
    }
    ...
    if(action->text() == "Label Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(5);
      } else {      
        p_treeWidget->hideColumn(5);
      }
    }
    ...
    if(action->text() == "Resolution Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(6);
      } else {      
        p_treeWidget->hideColumn(6);
      }
    }  

    if(action->text() == "Emission Angle Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(7);
      } else {
        p_treeWidget->hideColumn(7);
      }
    }

    if(action->text() == "Incidence Angle Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(8);
      } else {
        p_treeWidget->hideColumn(8);
      }
    }

    if(action->text() == "Notes Column") {
      if(action->isChecked()) {
        p_treeWidget->showColumn(9);
      } else {      
        p_treeWidget->hideColumn(9);
      }
    } 
  ...
  void MosaicWidget::displayControlPoints(){
    ...
    * 
    */
   void MosaicWidget::sortBy(QString sortby){
     //------------------------------------------
   ...
   bool sortResolution(QTreeWidgetItem *a,QTreeWidgetItem *b){
     return a->text(6).toDouble() < b->text(6).toDouble();
   }
   ...
   bool sortEmission(QTreeWidgetItem *a,QTreeWidgetItem *b){
     return a->text(7).toDouble() < b->text(7).toDouble();
   }
   ...
   bool sortIncidence(QTreeWidgetItem *a,QTreeWidgetItem *b){
     return a->text(8).toDouble() < b->text(8).toDouble();
   }
