...
namespace Qisis {
  ...
  void MosaicPointTool::findPoint (QPointF p, Isis::ControlNet *cn) {
    ...
    p_controlPoint = *(cn->Find(pointId.toStdString()));
    QString type = "no type";
    if (p_controlPoint.Type() == 0) {
      type = "Ground";
    } else {
      type = "Tie";
    }
    ...
    for (int m = 0; m < p_controlPoint.Size(); m++) {
      serialNumbers.append("\nControl Measure ");
      serialNumbers.append( QString::number(m+1));
      serialNumbers.append(" Serial #:  ");
      serialNumbers.append(QString::fromStdString(p_controlPoint[m].CubeSerialNumber()));
      ...
      for(int n = 0; n < items.size(); n++){
        if(p_controlPoint[m].CubeSerialNumber().compare(items[n]->serialNumber()) == 0) {
    ...
    p_pointInfoDialog->setText("Point ID: " + pointId 
                               + "\nNumber of Measures: " + QString::number(p_controlPoint.Size()) 
                               + serialNumbers +
                               + "\nPoint Type: " + type);
