 ...
 *                                    single quotes.
 *  @history 2007-02-22 Kris Becker - Added several options for dark processing.
 ...
 */
  class HiImageClean {
    ...
    private:
      ...
      typedef enum { Derived, Median, None } DarkOpt;

    public:
      HiImageClean() { }   // Renders it useless
      HiImageClean(const std::string &fname,
                   const std::string &darkopt = "DERIVED");
      HiImageClean(Cube &cube, const std::string &darkopt = "DERIVED");
      /** Destructor */
      ...
      void setDarkOption(const std::string &darkopt);
      double getDarkMedian() const { return (_darkMedian); }
    ...
    private:
      ...
      int                 _tdi;          //!< Time Delay Integration
      ...
      DarkOpt             _darkOption;    //!< Dark option in force
      H1DBuf              _predark;       //!< Dark correction w/out null fixes
      H1DBuf              _dark;          //!< Dark correction buffer
      H1DBuf              _darkApplied;   //!< Dark correction buffer used
      Statistics          _darkStats;     //!< Dark region statistics
      double              _darkMedian;    //!< Median of dark values
...
//  Private methods
      ...
      double calcMedian(const H1DBuf &data) const;
      void setDarkCorrect(const DarkOpt &option);
   };
};
