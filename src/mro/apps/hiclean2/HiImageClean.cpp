...
//  Declare constants for internal tracking
  ...
  const std::string HiImageClean::_revision = "$Revision: 1.5 $";
 ...
 */
  HiImageClean::HiImageClean(Cube &cube, const std::string &darkopt) {
    _filename = cube.Filename();
    _lines =  _samples = 0;
    _totalMaskNulled = _totalDarkNulled = 0;
    _darkMedian = 0.0;
    setDarkOption(darkopt);
    init(cube);
  }
...
/**
 * @brief Sets how dark current correction is handled
 * 
 * @param darkopt Option to apply - must be DERIVED, MEDIAN or NONE
 */
void HiImageClean::setDarkOption(const std::string &darkopt) {
  std::string opt = iString::UpCase(darkopt);
  DarkOpt d;

  if      (opt == "DERIVED") d = Derived;
  else if (opt == "MEDIAN")  d = Median;
  else if (opt == "NONE")    d = None;
  else {
    std::string mess = "Invalid DARKOPT specified (" + darkopt +
                       ") - must be DERIVED, MEDIAN or NONE";
...
//  mygroup.AddComment("Starting indexes");
    ...
    std::string darkopt("DERIVED");
    if (_darkOption == Median) darkopt = "MEDIAN";
    if (_darkOption == None) darkopt = "NONE";

    mygroup += PvlKeyword("DarkOption", darkopt);
    mygroup += PvlKeyword("DarkStartingColumn", 4);
    mygroup += PvlKeyword("DarkEndingColumn", 11);
    if (IsSpecial(_darkMedian)) {
      mygroup += PvlKeyword("DarkMedian");
    }
    else {
      mygroup += PvlKeyword("DarkMedian", _darkMedian);
    }
...
//  Use properties of H2DBuf to efficiently process the line.
    ...
    H2DBuf lbuf = column_apply(ibuf, _mask, _firstImageSample, _totalMaskNulled,
                               1.0);
    lbuf = row_apply(lbuf, _darkApplied, _firstImageLine+line, _totalDarkNulled,
                      1.0);
...
// this is a valid HiRISE image
    std::string instId = (std::string) instrument["InstrumentId"];
    if (!iString::Equal(instId,"HiRISE")) {
 ...
 */
  HiImageClean::H1DBuf HiImageClean::slice(const H2DBuf &buffer, long sample) const {
...
// Combine calibration region
  ...
  int calLines(_caldark.dim1());
  int ancLines(_ancdark.dim1());

  QuickFilter smoothCal(calLines, _filterWidth, 1);
  QuickFilter smoothAnc(ancLines, _filterWidth, 1);

  smoothCal.AddLine(&_predark[0]);
  smoothAnc.AddLine(&_predark[calLines]);

  nsamples = smoothCal.Samples() + smoothAnc.Samples();
  _dark = H1DBuf(nsamples);

  for (int s = 0 ; s < calLines ; s++) {
    _dark[s] = smoothCal.Average(s);
  }

  for (int t = 0 ; t < ancLines ; t++) {
    _dark[t+calLines] = smoothAnc.Average(t);
  }

  // Compute median and set specified algorithm
  _darkMedian = calcMedian(_dark);
  _darkApplied = _dark;
  setDarkCorrect(_darkOption);

  // Now apply to all calibration data
  BigInt nbad(0);
  _calimg  = row_apply(_calimg,  _darkApplied,               0, nbad, 1.0);
  _calbuf  = row_apply(_calbuf,  _darkApplied,               0, nbad, 1.0);
  _caldark = row_apply(_caldark, _darkApplied,               0, nbad, 1.0);
  _ancbuf  = row_apply(_ancbuf,  _darkApplied, _firstImageLine, nbad, 1.0);
  _ancdark = row_apply(_ancdark, _darkApplied, _firstImageLine, nbad, 1.0);
  return;
}
...
/**
 * @brief Computes the median from a 1D buffer
 * 
 * @param buf Input data buffer to compute median for
 * 
 * @return double Median of the data, NULL if no (valid) data
 */
double HiImageClean::calcMedian(const H1DBuf &buf) const {
  std::vector<double> data;
  data.reserve(buf.dim());
  ...
  if (data.size() == 0) return (NULL8);
  else return (data[data.size()/2]);
}

void HiImageClean::setDarkCorrect(const DarkOpt &option) {
  // Compute median and set specified algorithm
  if (option == Derived) {
    _darkApplied = _dark;
    _darkOption = option;
