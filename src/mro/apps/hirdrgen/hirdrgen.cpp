...
void IsisMain() {
  ...
  // and the output(y axis) vales are the original min and max for the cube.
  ...
  // Add the min/max per band keywords to the IMAGE object
  ...
  slope =  (p.GetOutputMaximum() - p.GetOutputMinimum()) / (maxmax - minmin);
  intercept = p.GetOutputMaximum() - slope * maxmax;
