...
SpiceDbGen::SpiceDbGen(iString type){
  ...
  * 
  * @param filter    The regular expression used to match kernels of a
  ...
  */
PvlObject SpiceDbGen::Direct(iString quality, iString location, iString filter){
  PvlObject result;
  
  //Create a list of all of the files
  QStringList files = GetFiles(Filename(location), filter);
  PvlObject pvlKernel("change");
  
  //throw an error if no files are being added to this database
  if (files.size() == 0){
    string message = "Your filter [" + location + "/" + filter + "]"
  ...
  if (type == "SPK"){result.SetName("SpacecraftPosition");}  
  else if (type == "CK"){result.SetName("SpacecraftPointing");}  
  return result;
}
...
//  const char* file = fileIn.Expanded().c_str(); 
  ...
  if (currFile == "SPK"){spkobj_c(tmp.c_str(), &currCell);}
  else if (currFile == "CK"){ckobj_c(tmp.c_str(), &currCell);}
  else if (currFile == "TEXT"){return PvlGroup("No coverage");}
