...
void IsisMain() {
  ...
  while (mpFile.GetLine(currLine)) {
    ...
    try{
      ...
      msg += ui.GetFilename("MATCH") + "] at line " + iString(line);
    ...
    if(snMap[(int)fsc].empty()) {      
      ...
      msg += ui.GetFilename("LIST2");
      msg += "] have an IMAGE_NUMBER or IMAGE_ID that matches the FSC [" + fsc;
      msg += "], from the match point file [" + ui.GetFilename("MATCH");
    ...
    try {
      ...
      iString msg = "Invalid match point file [" + ui.GetFilename("MATCH") + "]";
  ...
  // Added RAND file as input     //////////////////////////////

  // Open the RAND file
  if (ui.WasEntered("RAND")) {
    TextFile randFile(ui.GetFilename("RAND"));
    progress.SetText("Converting RAND file");
    ...
    while (randFile.GetLine(currLine)) {
      ...
      try {
        ...
        iString msg = "RAND file may not be valid.  Line count calculated [";
        msg += iString(inTotalLine) + "] for RAND file [" + ui.GetFilename("RAND");
      ...
      if (currLine.find("JULIAN") != string::npos) {
        // Since Progress MaximumSteps was approximated using the number of lines in the RAND file, 
      ...
      try{
        ...
        iString msg = "Invalid value(s) in RAND file [";
        msg += ui.GetFilename("RAND") + "] at line [" + iString(line);
      ...
      try {
        ...
        msg += iString(line) + "] of RAND file [" + ui.GetFilename("RAND") + "]";
        throw Isis::iException::Message (Isis::iException::User, msg, _FILEINFO_);
      }
    }
    // Update the Progress object
