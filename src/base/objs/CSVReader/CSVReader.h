...
class CSVParser {
  public:
    ...
    int size() const { return (_elements.dim()); }
     ...
     */
    const TokenType &operator()(const int nth) const { return (_elements[nth]); }
     ...
     */
    TokenList result() const { return (_elements); }
 ...
 * separated by a single character.  Consecutive deliminter characters can be
  ...
  class CSVReader {
    ...
    public:
      ...
      CSVReader(const std::string &csvfile, bool header = false, int skip = 0,
                const char &delimiter = ',', bool keepEmptyParts = true);
       ...
       * contain any text, but are presistantly ignored for all row and column
       * parsing operations.
       * 
       ...
       */
      void setSkip(int nskip) { if (nskip >= 0) _skip = nskip; }
       ...
       * maintainence.
       */
      bool haveHeader() const { return (_header); }
       ...
       *              one does not exist.
       */
      void setHeader(const bool gotIt = true) { _header = gotIt; }
       ...
       */
      void setDelimiter(const char &delimiter) { _delimiter = delimiter; }
       ...
       * fields.
       */
      void setKeepEmptyParts() { _keepParts = true; }
       ...
       */
      void setSkipEmptyParts() { _keepParts = false; }
       ...
       */
      bool keepEmptyParts() const { return (_keepParts); }
    ...
    private:
       ...
       */
      int firstRowIndex() const { return (_skip + ((_header) ? 1 : 0)); }
