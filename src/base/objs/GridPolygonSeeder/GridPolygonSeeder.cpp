...
namespace Isis {
  ...
  std::vector<geos::geom::Point*> GridPolygonSeeder::SeedGrid(const geos::geom::MultiPolygon *lonLatPoly, 
                                                              Projection *proj) {
   ...
   */
  std::vector<geos::geom::Point*> GridPolygonSeeder::SeedSubGrid(const geos::geom::MultiPolygon *lonLatPoly, 
                                                                 Projection *proj) {
    // Storage for the points to be returned
    std::vector<geos::geom::Point*> points;
      ...
      for(int y = 0; y < ySteps; y++) {
        ...
        for(int x = 0; x < xSteps; x++) {
          ...
          if(pointCheck[x][y] == pointShouldCheck) {
            p = CheckSubGrid(*xymp, centerX, centerY, 0);
          }
          ...
          //   square where a point was found, so check in depth
            ...
            p = CheckSubGrid(*xymp, centerX, centerY, precision);
          }
