...
namespace Isis {
  ...
  class Gruen : public AutoReg {
    public:
      ...
      inline double Gain() const { return (_result.Gain()); }
      /** Returns the radiometric shift value from the last solution */
      inline double Shift() const { return (_result.Shift()); }
    ...
    protected:
      ...
      virtual std::string AlgorithmName() const {return ("Gruen");}
    ...
    private:
      ...
      struct ErrorCounter {
        ...
        inline int  Errno() const { return (_gerrno); }
        inline BigInt Count() const { return (_count); }
        inline void BumpIt() { _count++; }
        PvlKeyword LogIt() const { return (PvlKeyword(_keyname, _count)); }
        int _gerrno;
        std::string _keyname;
        BigInt _count;
      };
      ...
      inline double getDefaultGain() const { return (_defGain); }
      /** Returns the default radiometric shift value */
      inline double getDefaultShift() const { return (_defShift); }
      void reset();
      void resetStats();
      ...
      AutoReg::RegisterStatus Status() const { return (Status(_result)); }
  };
