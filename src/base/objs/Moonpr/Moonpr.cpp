...
namespace Isis {
  Moonpr::Moonpr (Pvl &pvl) : PhotoModel(pvl) {
    p_photoM1 = -0.019;
    p_photoM2 = 0.000242;
  ...
  double Moonpr::PhotoModelAlgorithm (double phase, double incidence,
        double emission) {
    double pht_moonpr;
...
extern "C" Isis::PhotoModel *MoonprPlugin (Isis::Pvl &pvl) {
  return new Isis::Moonpr(pvl);
}
