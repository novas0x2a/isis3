...
namespace Isis {
  ...
  class ControlMeasure {
    public:
      ...
      double Sample() const { return p_sample; };

      //! Return the line coordinate of the measurement
      double Line() const { return p_line; };
       ...
       */
      double GetZScoreMin() const {return p_zScoreMin;};

      /** 
       * @brief Returns the maximum zScore
       * @return the maximum zScore
       */
      double GetZScoreMax() const {return p_zScoreMax;};

      //! Return error the sample coordinate of the measurement
      double SampleError() const { return p_sampleError; };

      //! Return error the line coordinate of the measurement
      double LineError() const { return p_lineError; };

      double ErrorMagnitude() const;

      //! Set how the coordinate was obtained
      void SetType(const MeasureType &type) { p_measureType = type; };

      //! Return the type of the measurment
      MeasureType Type () const { return p_measureType; };

      //! Has the measurement be measured??
      bool IsMeasured () const { return p_measureType != Unmeasured; };
      ...
      void SetReference(bool value) { p_isReference = value; };

      //! Is the measurement a reference?
      bool IsReference() const { return (p_isReference && IsMeasured()); };
       ...
       *            selected
       */
      void SetCubeSerialNumber(const std::string &sn) { p_serialNumber = sn; };

      //! Return the serial number of the cube containing the coordinate
      std::string CubeSerialNumber() const { return p_serialNumber; };
       ...
       */
      void SetDiameter(double diameter) { p_diameter = diameter; };
      ...
      std::string ChooserName() const { return p_chooserName; };
      ...
      void SetGoodnessOfFit(const double fit) { p_goodnessOfFit = fit; };

      //! Return the Goodnes of Fit
      double GoodnessOfFit() const { return p_goodnessOfFit; };

      //! Set the camera for this measure
      void SetCamera (Isis::Camera *camera) { p_camera = camera; };
      ...
      double FocalPlaneMeasuredX() const { return p_focalPlaneMeasuredX; };

      //! Return the measured focal plane y
      double FocalPlaneMeasuredY() const { return p_focalPlaneMeasuredY; };

      void SetFocalPlaneComputed(double x, double y);

      //! Return the computed focal plane x
      double FocalPlaneComputedX() const { return p_focalPlaneComputedX; };

      //! Return the computed focal plane y
      double FocalPlaneComputedY() const { return p_focalPlaneComputedY; };

      //! Set the measured ephemeris time of the measure
      void SetMeasuredEphemerisTime(double et) { p_measuredEphemerisTime = et; };

      //! Get the measured ephemeris time of the measure
      double MeasuredEphemerisTime() const { return p_measuredEphemerisTime; };

      //! Set the computed ephemeris time of the measure
      void SetComputedEphemerisTime(double et) { p_computedEphemerisTime = et; };
