 ...
 */ 
  ...
  class ProcessMosaic : public Isis::Process {
    ...
    public:
       ...
       */
      void SetBandBinMatch(bool pbFlag) { mbBandbinMatch = pbFlag; };
	  ...
	  void SetHighSaturationFlag(bool pbFlag) { mbHighSat = pbFlag;} ;
	  void SetLowSaturationFlag (bool pbFlag) { mbLowSat  = pbFlag;} ;
	  void SetNullFlag(bool pbFlag)           { mbNull    = pbFlag;} ;

	  bool GetHighSaturationFlag(void) { return mbHighSat;} ; 
	  bool GetLowSaturationFlag (void) { return mbLowSat;}  ;
	  bool GetNullFlag(void)           { return mbNull;}    ;

	  // Set the priority input, mosaic, band
	  void SetPriority(MosaicPriority pePriority) { mePriority = pePriority;};
	 
	  // Set/Get the Track Flag 
	  void SetTrackFlag(bool pbFlag)   { mtTrackInfo.bTrack= pbFlag; };
	  bool GetTrackFlag(void)		   { return mtTrackInfo.bTrack;  };	  
	  ...
	  void SetCreateFlag(bool pbFlag)  { mtTrackInfo.bCreate  = pbFlag;};

	  // Set the Band Number for priority Band
	  void SetBandNumber(int piBandNum){ mtTrackInfo.iBandNum = piBandNum; };

	  // Set the keyword for priority Band
	  void SetBandKeyWord(std::string psKeyName, std::string psKeyValue) {mtTrackInfo.sKeyName=psKeyName; mtTrackInfo.sKeyValue=psKeyValue;};

	  // Set the Band Criteria Lesser/Greater than for Band priority
	  void SetBandCriteria(BandCriteria peCriteria) {mtTrackInfo.eCriteria=peCriteria;};
	  
	  void Test(void);  

	  int GetInputStartLine(void)   { return miOsl; };
	  int GetInputStartSample(void) { return miOss; };
	  int GetInputStartBand(void)   { return miOsb; };
