        ...
        for (int i=0; i<iNumObjs; i++) {
          ...
          if (cPvlObj.HasKeyword("Name", Pvl::Traverse)) {      
              ...
              if (cNameKey[0]==sTableName) {              
                ...
                bool bFull=false;
                switch (sizeof(OutputCubes[0]->PixelType())){
                  case 1: if (iRecs >= (VALID_MAX1 - 1))               // Index is 1 based as 0=Null invalid value
                        bFull = true;
                        break;
                  case 2: if (iRecs >  (VALID_MAX2 - VALID_MIN2 + 1))  // Signed 16bits with some special pixels
                        bFull = true;
                        break;
                      
                  case 4: if (iRecs >  (FLOAT_MAX - FLOAT_MIN + 1))    // Max float mantissa
                        bFull = true;
                        break;      
                }
