 ...
 * 
 * @param openmode Open Mode of file to be opened. InputOpens file for Input: 
 *                 Read Only OutputOpens file for Output: Read / Write - 
 *                 Truncates if file exists. AppendOpens file for Append: 
 *                 Read / Write - Appends if file exists.Defaults to "input"
 * 
 ...
 */
 void TextFile::Open(const std::string &filename, const char *openmode,
  ...
  // note: in append mode, the input and output pointers move together
    ...
    if      (chkOpenmode == "input")      {p_openmode = 1;}
    else if (chkOpenmode == "output")     {p_openmode = 2;}
    else if (chkOpenmode == "overwrite")  {p_openmode = 3;}
    else if (chkOpenmode == "append")     {p_openmode = 4;}
    else {
      string message = "TextFile::-> Unknown openmode: (input, output, overwrite, append):["
                     + string(openmode) + "]:[" + filename + "]";
      throw Isis::iException::Message(Isis::iException::Programmer,
                                      message,_FILEINFO_);
    }
  
    if (p_openmode == 1) {                                     // input
      p_stream.open (p_filename.c_str(), fstream::in);
    }
    else if (p_openmode == 2) {                                // output
      // first chk if file already exists
      p_stream.open (p_filename.c_str(), fstream::in);
        if (p_stream.is_open()) {
    ...
    else if (p_openmode == 3) {                                // overwrite
      p_stream.open (p_filename.c_str(), fstream::in | fstream::out | fstream::trunc);
    }
    else if (p_openmode == 4) {                                // append
  ...
  void TextFile::Rewind () {
    OpenChk(true);
    if (p_stream.eof()) {p_stream.clear();}
                         ...
                         const bool skipComments)
  {
    ...
    while (GetLine(line, skipComments)) {
      ...
      else if (lines[lineCount] == "\0") {break;}
  ...
  void TextFile::PutFile(std::vector<std::string> &lines, const int &maxLinesToWrite)
  {
    ...
    for(int lineCount=0; lineCount < (int) lines.size(); lineCount++) {
      if (maxLinesToWrite > 0) {
        if (lineCount > maxLinesToWrite){break;};
      }
  ...
  */
  int TextFile::LineCount(const int &maxLinesToRead) {
    ...
    if (maxLinesToRead > 0) {
      ...
      while (getline(p_stream, tmpLine)) {lineCount++;}
    }
