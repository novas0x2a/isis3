...
int main (void)
{
  ...
  cout << "Unit Test for UvvisCamera..." << endl;
  /**
   * Increased line/samp->lat/lon->line/samp tolerances
   */
  try{
    // These should be lat/lon at center of image. To obtain these numbers for a new cube/camera,
    // set both the known lat and known lon to zero and copy the unit test output "Latitude off by: "
    // and "Longitude off by: " values directly into these variables.
    double knownLat = -78.56557059110476;
    double knownLon = 126.7899573707551;
...
void TestLineSamp(Isis::Camera *cam, double samp, double line) {
  ...
  if(success) {
    ...
    if (fabs(deltaSamp) < 0.002) deltaSamp = 0;
    if (fabs(deltaLine) < 0.002) deltaLine = 0;
